...
def naivebayes(train, test, a, pred_file):
  """ remove punctuation """
  ...
  def make_features(vectorizer, data):
    ...
    feature = pd.concat([data[['left', 'top', 'width', 'height', 'page', 'num']].fillna(0), feature], axis=1)
  ...
  bayes.fit(train_feature, train['label'])
  
  """ save pred """
  test_feature = make_features(vectorizer, test)
  pred = bayes.predict_proba(test_feature)
  ...
  print('***NaiveBayes finished without error***')
